...
/* *** Pseudo Header *** */
  ...
  ps_iphdr->tcpl =
    htons (sizeof (struct tcphdr2) + sizeof (struct tcp_opthdr));
...
/* dest address */
  ...
  for (;;)
      ...
      if (srcport == 0)
	{
      ...
      if (srcip == 0)
        {                       /* random source */
          xf_iphdr->ip_src.s_addr = rand () % 1006632960;
          ps_iphdr->saddr = xf_iphdr->ip_src.s_addr;
        }
...
/* parse arguments */
  if (argc != 6)
    {
      printf ("usage : %s dest src destport srcport flags\n\n", argv[0]);
